...
function setupParallax() {
  ...
  window.addEventListener(
    ...
    (event) => {
      ...
      elements.forEach((element) => updateElement(element, x, y));
    },
    { passive: true }
  );
}
...
function setupFloatingAssistant() {
  ...
  const animateThrow = () => {
    ...
    const step = () => {
      ...
      velocityX += dragOffsetX * -0.028;
      velocityY += dragOffsetY * -0.028;
      velocityX *= 0.9;
      velocityY *= 0.9;
      ...
      const settled =
        Math.abs(nextOffset.x) < 0.8 &&
        Math.abs(nextOffset.y) < 0.8 &&
        Math.abs(velocityX) < 0.18 &&
        Math.abs(velocityY) < 0.18;
  ...
  const updateScrollState = () => {
    ...
    assistant.classList.toggle("is-interactive", touchMode && assistantActive);
  ...
  if (touchMode) {
    assistant.addEventListener("pointerdown", (event) => {
      if (!assistantActive || event.pointerType === "mouse") {
        return;
      }
    ...
    const releaseDrag = (event) => {
      ...
      dragPointerId = null;
      assistant.classList.remove("is-dragging");
      velocityX = clamp(velocityX, -34, 34);
      velocityY = clamp(velocityY, -34, 34);
      animateThrow();
    };
