...
function setupFloatingAssistant() {
  ...
  const gamePanel = document.getElementById("assistant-game");
  const walletToggle = document.getElementById("assistant-wallet-toggle");
  const shopPanel = document.getElementById("assistant-shop");
  const coinCount = document.getElementById("assistant-coin-count");
  const shopButtons = Array.from(document.querySelectorAll(".assistant-shop-item[data-effect-buy]"));
  const effectLayer = document.getElementById("assistant-effect-layer");
  const reducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
  const clamp = (value, min, max) => Math.min(Math.max(value, min), max);
  const storageKey = "assistant-coin-balance-v1";
  const effectCatalog = {
    fireworks: { symbols: ["🎆", "✨", "💥"], count: 44, spreadX: 320, spreadY: 250, duration: 1650, fullPage: true },
    hearts: { symbols: ["💖", "💗", "💕"], count: 36, spreadX: 240, spreadY: 180, duration: 1500, fullPage: true },
    stars: { symbols: ["⭐", "🌟", "✨"], count: 34, spreadX: 250, spreadY: 190, duration: 1450, fullPage: true },
    thumbs: { symbols: ["👍", "👌", "🔥"], count: 32, spreadX: 230, spreadY: 175, duration: 1420, fullPage: true },
    icons: { symbols: ["🎯", "🎮", "🚀"], count: 36, spreadX: 245, spreadY: 190, duration: 1480, fullPage: true },
    faces: { symbols: ["😄", "😎", "🤩"], count: 34, spreadX: 215, spreadY: 170, duration: 1360, fullPage: true },
    captureBonus: { symbols: ["💰", "✨", "🎉"], count: 12, spreadX: 210, spreadY: 165, duration: 1280 },
  };
  ...
  if (!assistant || !hero || !heroGrid || !footer) {
    return;
  }
  ...
  const saveCoins = () => {
    try {
      window.localStorage.setItem(storageKey, String(coinBalance));
    } catch (error) {
      // Ignore localStorage errors (privacy mode / quota).
    }
  };

  const getButtonPrice = (button) => {
    const parsed = Number.parseInt(button.dataset.price || "0", 10);
    return Number.isFinite(parsed) && parsed > 0 ? parsed : 0;
  };
  ...
  const updateShopState = () => {
    if (coinCount) {
      coinCount.textContent = String(coinBalance);
    }

    shopButtons.forEach((button) => {
      const price = getButtonPrice(button);
      const isLocked = price > coinBalance;
  ...
  const syncShopVisibility = () => {
    const canShowShop = assistantActive && shopOpen;

    gamePanel?.classList.toggle("is-shop-open", canShowShop);
    walletToggle?.setAttribute("aria-expanded", String(canShowShop));
    shopPanel?.setAttribute("aria-hidden", String(!canShowShop));
  };
  ...
  const spawnEffect = (effectType, originX = window.innerWidth * 0.5, originY = window.innerHeight * 0.56) => {
    if (!effectLayer) {
      return;
    }

    const config = effectCatalog[effectType];

    if (!config) {
      return;
    }

    const particles = reducedMotion ? Math.max(8, Math.floor(config.count * 0.45)) : config.count;
    const fullPageBurst = Boolean(config.fullPage);
    const viewportPadding = 18;

    for (let index = 0; index < particles; index += 1) {
      const symbol = config.symbols[Math.floor(Math.random() * config.symbols.length)];
      const angle = Math.random() * Math.PI * 2;
      const startX = fullPageBurst
        ? viewportPadding + Math.random() * Math.max(window.innerWidth - viewportPadding * 2, 1)
        : originX + (Math.random() - 0.5) * 66;
      const startY = fullPageBurst
        ? viewportPadding + Math.random() * Math.max(window.innerHeight - viewportPadding * 2, 1)
        : originY + (Math.random() - 0.5) * 52;
      const travelX = fullPageBurst
        ? Math.cos(angle) * (config.spreadX * (0.25 + Math.random() * 0.55))
        : Math.cos(angle) * (config.spreadX * (0.4 + Math.random() * 0.6));
      const travelY = fullPageBurst
        ? Math.sin(angle) * (config.spreadY * (0.25 + Math.random() * 0.55))
        : -Math.abs(Math.sin(angle)) * (config.spreadY * (0.5 + Math.random() * 0.5))
          + (Math.random() - 0.5) * 52;
      const item = document.createElement("span");

      item.className = "assistant-effect-item";
      item.textContent = symbol;
      item.style.setProperty("--start-x", `${startX}px`);
      item.style.setProperty("--start-y", `${startY}px`);
      item.style.setProperty("--travel-x", `${travelX}px`);
      item.style.setProperty("--travel-y", `${travelY}px`);
      item.style.setProperty("--spin", `${(Math.random() - 0.5) * 120}deg`);
      item.style.setProperty(
        "--effect-duration",
        `${Math.round(config.duration * (0.8 + Math.random() * 0.35))}ms`
      );
      effectLayer.appendChild(item);
      window.setTimeout(() => item.remove(), config.duration + 900);
    }
  };

  const purchaseEffect = (button) => {
    const effectType = button.dataset.effectBuy;
    const price = getButtonPrice(button);

    if (!effectType || !effectCatalog[effectType] || price <= 0) {
      return;
    }

    if (!spendCoins(price)) {
      showAssistantBubble(`Ti servono almeno ${price} monete`, 1900);
      return;
    }

    const itemName = button.querySelector(".assistant-shop-name")?.textContent?.trim() || "Effetto";
    showAssistantBubble(`${itemName} attivati! -${price} monete`, 1800);
  ...
  const captureAssistant = () => {
    ...
    resetOrientation();
    changeCoins(10);
    showAssistantBubble("Presa! +10 monete", 2200);
    spawnEffect("captureBonus", rect.left + rect.width * 0.5, rect.top + rect.height * 0.4);
    return true;
  };
  ...
  const animateThrow = () => {
    ...
    const step = (time) => {
      ...
      const delta = lastThrowTime
        ...
        : 1;
      const bounds = getDragBounds();
      const friction = Math.pow(0.991, delta);
      ...
      isCapturable = Math.hypot(velocityX, velocityY) > 0.55;

      const settled = Math.hypot(velocityX, velocityY) < 0.34;
  ...
  const updateScrollState = () => {
    ...
    const reachedPageEnd = window.scrollY + window.innerHeight >= document.documentElement.scrollHeight - 34;
    const shouldShowFooter = footerProgress > 0.06 || reachedPageEnd;
    ...
    gamePanel?.classList.toggle("is-visible", assistantActive);
    footer.classList.toggle("is-visible", shouldShowFooter);
    syncShopVisibility();

    if (assistantActive && !assistantWasActive && !introHintShown) {
      showAssistantBubble(
        "Iniza a giocare lanciandomi e prendendomi.",
        6200
      );
      introHintShown = true;
    }
  ...
  document.addEventListener("click", (event) => {
    if (!shopOpen || !gamePanel) {
      return;
    }

    if (gamePanel.contains(event.target)) {
      return;
    }
  ...
  assistant.addEventListener("pointerdown", (event) => {
    ...
    if (throwFrame) {
      ...
      if (now >= captureCooldownUntil && captureAssistant()) {
        captureCooldownUntil = now + 220;
        return;
      }
    }
  ...
  const releaseDrag = (event) => {
    ...
    dragPointerId = null;
    assistant.classList.remove("is-dragging");
    velocityX = clamp(velocityX * 1.35, -74, 74);
    velocityY = clamp(velocityY * 1.35, -74, 74);
    animateThrow();
  };
